  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link href="/admin" className="text-sm text-brand-600 underline">
          ← Dashboard
        </Link>
      </div>
                ...
                <tr key={port.unlocode} className="hover:bg-zinc-50">
                  ...
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-2">
                      <Link
                        href={`/admin/ports/${port.unlocode}`}
                      ...
                      {usageCount === 0 && (
                        <PortDeleteButton
                          ...
                          name={port.name}
                          locale={locale}
                          backHref={`/${locale}/admin/ports${q ? `?q=${encodeURIComponent(q)}` : ""}`}
                        />
                      )}
                    </div>
                  </td>
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex items-center justify-between text-sm text-zinc-500">
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link
                href={`/admin/ports?${q ? `q=${encodeURIComponent(q)}&` : ""}page=${page - 1}`}
