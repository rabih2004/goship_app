...
import {
  startCustomsOnboardingAction,
  refreshCustomsAccountStatusAction,
  mockResetCustomsOnboardingAction,
} from "./actions";

export default async function CustomsOnboardingPage({
  params,
  searchParams,
}: {
    ...
    <div className="mx-auto w-full max-w-2xl px-6 py-12">
      ...
      <div className="rounded-lg border border-zinc-200 bg-white p-6">
        ...
        <div className="flex flex-wrap gap-2">
          {stage !== "complete" && (
            <form action={startCustomsOnboardingAction}>
          ...
          {!mockMode && profile.stripeAccountId && (
            <form action={refreshCustomsAccountStatusAction}>
              <input type="hidden" name="locale" value={locale} />
          ...
          {mockMode && stage === "complete" && (
            <form action={mockResetCustomsOnboardingAction}>
              <input type="hidden" name="locale" value={locale} />
              <Button type="submit" variant="secondary">
                {t("mockResetBtn")}
              </Button>
