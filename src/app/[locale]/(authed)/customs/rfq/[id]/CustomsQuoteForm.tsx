...
import {
  submitCustomsQuoteAction,
  type SubmitCustomsQuoteState,
} from "./actions";

const initialState: SubmitCustomsQuoteState = { ok: false };

export function CustomsQuoteForm({
  shipmentId,
  locale,
  suggestedUSD,
}: {
  ...
  const t = useTranslations("CustomsRfq");
  ...
  return (
    <form action={action} className="flex flex-col gap-4">
      ...
      <div className="rounded-md border border-zinc-200 bg-zinc-50 p-3 text-xs text-zinc-700">
        {t("suggestionLine", { price: suggestedUSD.toFixed(2) })}
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div>
          <Label htmlFor="etaDays">{t("etaDays")}</Label>
          <Input
            id="etaDays"
            name="etaDays"
            type="number"
            ...
            invalid={!!fe.etaDays}
          />
          <FieldError>{fe.etaDays}</FieldError>
        </div>
        <div>
