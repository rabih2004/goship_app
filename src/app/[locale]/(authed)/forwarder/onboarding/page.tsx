...
import {
  ...
  refreshAccountStatusAction,
  mockResetOnboardingAction,
} from "./actions";
...
export default async function OnboardingPage({
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-6 py-12">
      <div className="mb-6">
        <Link href="/forwarder" className="text-sm text-[var(--brand)] underline">
          ← {t("backToDashboard")}
        </Link>
      </div>
      ...
      <div className="rounded-lg border border-zinc-200 bg-white p-6">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <div className="font-medium text-zinc-900">{t(`stage.${stage}.title`)}</div>
            <div className="text-sm text-zinc-500">{t(`stage.${stage}.body`)}</div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {!mockMode && profile.stripeAccountId && (
            <form action={refreshAccountStatusAction}>
              <input type="hidden" name="locale" value={locale} />
          ...
          {mockMode && stage === "complete" && (
            <form action={mockResetOnboardingAction}>
              <input type="hidden" name="locale" value={locale} />
              <Button type="submit" variant="secondary">
                {t("mockResetBtn")}
              </Button>
...
function StatusDot({ stage }: { stage: "not-started" | "in-progress" | "complete" }) {
  const cls =
    stage === "complete"
      ? "bg-emerald-500"
      : stage === "in-progress"
